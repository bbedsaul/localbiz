...
3. From the monorepo root, with Node 20:
   ...
   pnpm --filter @sitevitals/web dev         # http://localhost:3000
   ```
...
Root directory: repo root; build command `pnpm --filter @sitevitals/engine build && pnpm --filter @sitevitals/web build`; output handled by Next. Set all env vars in the Vercel project. The Supabase magic-link redirect URL must include `https://<your-vercel-domain>/auth/callback`.
